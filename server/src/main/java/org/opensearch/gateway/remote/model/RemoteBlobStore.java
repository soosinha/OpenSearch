 ...
 * @param <U> The concrete class implementing {@link RemoteEntity} which is used as a wrapper for T entity.
 */
public class RemoteBlobStore<T extends AbstractRemoteBlobEntity<T>> implements RemoteEntityStore<T> {
    ...
    public RemoteBlobStore(BlobStoreTransferService blobStoreTransferService, BlobStoreRepository blobStoreRepository, String clusterName,
    ...
    public void writeAsync(T obj, ActionListener<Void> listener) {
        assert obj.get() != null;
        try {
            try (InputStream inputStream = obj.serialize()) {
    ...
    private BlobPath getBlobPathForUpload(AbstractRemoteBlobEntity<T> obj) {
    ...
    private BlobPath getBlobPathForDownload(AbstractRemoteBlobEntity<T> obj) {
