...
public class RemoteClusterStateServiceTests extends OpenSearchTestCase {
    ...
    public void testFailWriteFullMetadataWhenRemoteStateDisabled() throws IOException {
        final DiscoveryNodes nodes = DiscoveryNodes.builder().clusterManagerNodeId("cluster-manager-id").localNodeId("cluster-manager-id").build();
    ...
    public void testWriteFullMetadataSuccess() throws IOException {
        ...
        final ClusterMetadataMarker expectedMarker = ClusterMetadataMarker.builder().indices(indices).term(1L).version(1L).stateUUID("state-uuid").clusterUUID("cluster-uuid").build();
    ...
    public void testFailWriteIncrementalMetadataWhenTermChanged() {
        ...
        final ClusterState previousClusterState = ClusterState.builder(ClusterName.DEFAULT).metadata(Metadata.builder().coordinationMetadata(coordinationMetadata)).build();
        assertThrows(AssertionError.class, () -> remoteClusterStateService.writeIncrementalMetadata(previousClusterState, clusterState, null));
    }
    ...
    public void testWriteIncrementalMetadataSuccess() throws IOException {
        ...
        final ClusterMetadataMarker marker = remoteClusterStateService.writeIncrementalMetadata(previousClusterState, clusterState, previousMarker);
        final UploadedIndexMetadata uploadedIndexMetadata = new UploadedIndexMetadata("test-index", "index-uuid", "metadata-filename");
        Map<String, UploadedIndexMetadata> indices = Map.of("test-index", uploadedIndexMetadata);
    ...
    private static ClusterState generateClusterState() {
        ...
        return ClusterState.builder(ClusterName.DEFAULT).version(1L).stateUUID("state-uuid")
            .metadata(Metadata.builder().put(indexMetadata, true).clusterUUID("cluster-uuid").coordinationMetadata(coordinationMetadata).build()).build();
    }
